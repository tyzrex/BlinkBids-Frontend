...
export type ChildCategoryTable = {
  name: string;
  slug: string;
  label: string;
  id: number;
...
export const deleteCategoryById = async (id: number) => {
  const response = await removeCategory(id, "child");
...
export const columns: ColumnDef<ChildCategoryTable>[] = [
...

  {
    ...
    cell: ({ row }) => {
      return (
        <div>
          <Link
            href={`/admin/category-children/edit-category?name=${row.original.name}`}
