...
import { getParentCategories } from "@/api/cms";
...
export default async function CategoriesTable({
  ...
  const [error, categories] = await goTry(getParentCategories(session, page));
  if (error || !categories)
    return (
  ...
  return (
    <>
      <div>
        <div className="py-5">
          <h1 className="title-typography">Product Parent Categories</h1>
        </div>
