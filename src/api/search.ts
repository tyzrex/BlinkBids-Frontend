...
export const searchProducts = async (params: ISearchParams) => {
    try {
        ...
        const url = `search/?query=${params?.query}&page=${params?.page || 1}${sortParam}${minPriceParam}${maxPriceParam}${brandParam}${categoryParam}`;
...
export const searchRecomendations = async (
    searchInput: string
) => {
    try {
        const response = await GetRequest(`/search?query=${searchInput}`);
        return response.data;
    } catch (error) {
        //console.log(error);
